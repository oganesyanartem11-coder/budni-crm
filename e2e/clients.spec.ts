...
test('селект курьера присутствует в строке точки @smoke', async ({ page }) => {
  ...
  // <option> в HTML технически "hidden" пока <select> не раскрыт.
  // Проверяем что select существует и содержит опцию "Не назначен" — это
  // гарантирует что курьерская выпадашка отрендерилась корректно.
  const courierSelect = page
    .locator('select')
    .filter({ has: page.locator('option', { hasText: 'Не назначен' }) })
  await expect(courierSelect.first()).toBeAttached()
  await expect(courierSelect.first()).toContainText('Не назначен')

 ...
 */
export type WelcomeKind = 'FIXED' | 'DYNAMIC' | 'SAMEDAY' | 'WEEKLY'
 ...
 * 2. Иначе есть хоть один конфиг orderType='WEEKLY' → 'WEEKLY' (недельная заявка).
 * 3. Иначе есть хоть один конфиг orderType='DYNAMIC' → 'DYNAMIC'.
 * 4. Иначе (только FIXED-конфиги или конфигов нет) → 'FIXED' (нейтральный).
 */
export function pickWelcomeKind(client: ClientForWelcome): WelcomeKind {
  ...
  if (client.mealConfigs.some((c) => c.orderType === 'WEEKLY')) return 'WEEKLY'
...
export const WELCOME_WEEKLY = `Здравствуйте! Это Будни. Подключили вас к каналу для оперативной связи.

Раз в неделю присылайте сюда заявку на следующую неделю (Пн–Вс) — фото бумажки или текст со списком дат и количеств. Принимаем до воскресенья. В четверг напомним если ещё нет.

В течение недели можно корректировать, отменять отдельные дни или просто что-то спросить — пишите. Команда видит, ответим.

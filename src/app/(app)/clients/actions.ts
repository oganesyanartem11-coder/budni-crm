...
export async function createMealConfigBulk(
  ...
  // Если конфиг FIXED — попробуем сразу сгенерировать заказ на завтра
  // (для случаев когда конфиг создан днём и ждать утреннего cron'а нерационально)
  let autoGenerated = 0
  if (parsed.data.orderType === 'FIXED') {
    try {
      const { generateFixedOrdersForDate } = await import('@/lib/orders/generate-fixed')
      ...
      // Не ронять создание конфига если автогенерация заглохла

...
export async function updateMealConfig(
  ...
  // 7.41: затронутые будущие заказы при смене fixedPortions. Расширено:
  // - CONFIRMED тоже (FIXED-заказы создаются сразу CONFIRMED — раньше B-5 их не
  //   ловил и был фактически no-op для FIXED, ради которого и затевался);
  // - deliveryDate >= ЗАВТРА (сегодняшний заказ не трогаем — он уже в работе);
  // - только авто-источники (FIXED_AUTO/RECURRING_AUTO) — MANUAL правил человек,
  //   не перетираем его значения.
  const affectedOrdersWhere: Prisma.OrderWhereInput = {
    sourceConfigId: id,
  ...
  if (portionsChanged && parsed.data.confirmDraftPortions === 'update') {
    // Читаем затронутые заказы (id + pricePerPortion). pricePerPortion
    // индивидуален per-order (snapshot), поэтому totalPrice пересчитываем
    // отдельно на каждый. С 7-дневным горизонтом (7.41) заказов может быть
    // много — обновляем партиями по 100, каждая партия в своей транзакции.
    const orders = await prisma.order.findMany({
      where: affectedOrdersWhere,
      select: { id: true, pricePerPortion: true },
    })

    // ВАЖНО: конфиг обновляем ПОСЛЕ заказов. Если цикл упадёт на середине,
    // конфиг останется со старым fixedPortions → повторный сабмит снова поднимет
    // B-5 (old≠new) и докрутит оставшиеся заказы (UPDATE идемпотентен).
...
export async function createMealConfigBulk(
  ...
  // 7.41: при создании конфига сразу пролонгируем на 7 дней вперёд (как cron),
  // чтобы планирование было видно заранее. DYNAMIC → PENDING_CONFIRMATION,
  // FIXED → CONFIRMED; same-day-конфиги обрабатываются внутри на сегодня.
  let autoGenerated = 0
  try {
    const { generateFixedOrdersForRange } = await import('@/lib/orders/generate-orders')
    const tomorrow = getMskCalendarDayUtc(new Date(), 1)
    const stats = await generateFixedOrdersForRange(tomorrow, 7, { triggeredByUserId: null })
